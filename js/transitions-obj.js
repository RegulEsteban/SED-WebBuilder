window.kontext_obj = function( container ) {

	// Dispatched when the current layer changes
	var changed = new kontext_obj.Signal();

	// All layers in this instance of kontext
	var layers = Array.prototype.slice.call( container.querySelectorAll( '.layer-obj' ) );
 ...
 */
kontext_obj.Signal = function() {
	this.listeners = [];
}

kontext_obj.Signal.prototype.add = function( callback ) {
	this.listeners.push( callback );
}

kontext_obj.Signal.prototype.remove = function( callback ) {
	var i = this.listeners.indexOf( callback );

	if( i >= 0 ) this.listeners.splice( i, 1 );
}

kontext_obj.Signal.prototype.dispatch = function() {
